...
from ptsplitter.utils import (
    positive_edges,
    negative_edges,
    iter_get_scores_networkx
)


print('Reading in dataset.')
G = max(nx.connected_component_subgraphs(nx.read_edgelist('data_input/CA-AstroPh.txt')), key=len)
...
positive_scores_persona = [
    excepts(ValueError, max, lambda _: 0.0)(iter_get_scores_networkx(groups, node1, node2, PG, jaccard_coefficient))
