...
class SplitterEmbedding(nn.Module):
    def __init__(self,
                 ...
                 initial_persona_embedding=None) -> None:  # TODO add types
        ...
        if initial_embedding is None:
            initial_embedding = torch.ones(node_count, embedding_dimension)  # TODO change to random
        else:
            initial_embedding = torch.from_numpy(initial_embedding)
        if initial_persona_embedding is None:
            initial_persona_embedding = torch.ones(persona_node_count, embedding_dimension)  # TODO change to random
        else:
            initial_persona_embedding = torch.from_numpy(initial_persona_embedding)
        self.persona_embedding.weight.data = torch.nn.Parameter(initial_persona_embedding)
        self.embedding.weight.data = torch.nn.Parameter(initial_embedding, requires_grad=False)
